...
test "Longest Run" {
    ...
    const stat = try zsts.longestRun.longestRunDetectStatDetect(allocator, param, 8);
    stat.init(&param);
    const result = stat.iterate(bytes);
    ...
    try std.testing.expect(result.v_value >= 5.243977);
    try std.testing.expect(result.p_value >= 0.127563);
    try std.testing.expect(result.q_value <= 0.127565);
}
...
test "Binary Derivative" {
    ...
    try std.testing.expect(result.passed == true);
    try std.testing.expect(result.v_value >= 0.357770);
    try std.testing.expect(result.p_value >= 0.720514);
    try std.testing.expect(result.q_value <= 0.360258);
}
...
test "autocorrelation" {
    ...
    try std.testing.expect(result.v_value >= -0.266208);
    try std.testing.expect(result.p_value >= 0.790079);
    try std.testing.expect(result.q_value <= 0.604961);
}
